...
public class DOIIdentifierProvider extends FilteredIdentifierProvider {
    ...
    public void delete(Context context, DSpaceObject dso)
        ...
        try {
            String doi = getDOIOutOfObject(context, dso);

            while (null != doi) {
                this.removeDOIFromObject(context, dso, doi);
                doi = getDOIOutOfObject(context, dso);
            }
...
//                "AND resource_id = ? AND ((status != ? AND status != ?) OR status IS NULL)";
    ...
    protected DOI loadOrCreateDOI(Context context, DSpaceObject dso, String doiIdentifier, Filter filter)
        ...
        if (null != doiIdentifier) {
            ...
            String identifierSuffix = DoiGenerationStrategy
                .getApplicableStrategy(context, doiGenerationStrategies,
                    (Item)dso).generateDoi(context, (Item)dso, doi);
            doiIdentifier = this.getPrefix() + "/" + identifierSuffix;
        }
     ...
     */
    public String getDOIOutOfObject(Context context, DSpaceObject dso) throws DOIIdentifierException {
        // FIXME
        if (!(dso instanceof Item)) {
    ...
    public void setDoiGenerationStrategies(Set<DoiGenerationStrategy> doiGenerationStrategies) {
        this.doiGenerationStrategies = doiGenerationStrategies;
    }
